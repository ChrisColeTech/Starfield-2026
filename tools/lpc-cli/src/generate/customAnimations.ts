...
export interface CustomAnimationArea {
  name: string;
...
export const ANIMATION_ROWS_LAYOUT: Record<string, number> = {
  "thrust-n": 3,
  "thrust-w": 4,
  "thrust-s": 5,
  "thrust-e": 6,
...
export const CUSTOM_ANIMATIONS: Record<string, CustomAnimationDefinition> = {
  wheelchair: {
    frameSize: 64,
    frames: [
      ["sit-n,2", "sit-n,2"],
...
export function customAnimationBase(definition: CustomAnimationDefinition): string {
...
export function customAnimationArea(name: string, definition: CustomAnimationDefinition, yOffset: number): CustomAnimationArea {
  ...
  for (const [rowIndex, row] of definition.frames.entries()) {
    if (row.length !== columns) {
      throw new Error(
        `Custom animation '${name}' row ${rowIndex} has ${row.length} columns, expected ${columns}.`
      );
    }
  }
  if (definition.frameSize < FRAME_SIZE) {
    throw new Error(
      `Custom animation '${name}' frameSize ${definition.frameSize} is smaller than standard frame size ${FRAME_SIZE}.`
    );
  }
