...
public class AnimeModelsScreen : IDisposable
{
    ...
    public string StatusText { get; private set; } = "No anime map loaded";
    ...
    public void LoadAnimeModels(string animeTreesPath, string? preferredMapId = null)
    {
        Func<MapDefinition, bool> animeFilter = m =>
            m.Id.Contains("anime", StringComparison.OrdinalIgnoreCase)
            || m.Name.Contains("anime", StringComparison.OrdinalIgnoreCase);

        StatusText = _map.Load(preferredMapId ?? "anime_forest_map", "anime_forest", animeFilter,
            "No anime map found", m => $"Anime Forest ({m.Width}x{m.Height})");
    }
    ...
    public bool SwitchMap(int direction)
    {
        ...
        bool changed = _map.SwitchMap(direction, animeFilter, out var status,
            m => $"Anime Forest ({m.Width}x{m.Height})");
    ...
    public void LoadCharacter(string folderPath)
    {
        bool ok = _actor.LoadCharacter(folderPath, LoadMode, FillTags, SharedAnimationFolders,
            PokemonRoot, TrainerParties, "AnimeScreen");
        string charName = System.IO.Path.GetFileName(folderPath);
        StatusText = ok ? $"Anime Forest | {charName}" : $"Anime Forest | Failed: {charName}";
    }
