...
public class FollowCamera
{
    ...
    private float _smoothedObstacleBoost;
    private float _obstacleBoostVelocity;
    ...
    private const float ObstacleProbeRadius = 8f;
    private const int ObstacleProbePoints = 12;
    private const float ObstacleBoostRiseSmoothTime = 0.4f;
    private const float ObstacleBoostFallSmoothTime = 2.0f;
    ...
    public void Initialize(Vector3 target)
    {
        ...
        _elevVelocity = 0f;
        _smoothedObstacleBoost = 0f;
        _obstacleBoostVelocity = 0f;
    }
        ...
        float inputYaw, float inputPitch, float inputZoom)
    {
        ...
        // Camera yaw ONLY follows when actively moving forward (pressing up).
        // Otherwise completely frozen — no tracking, no velocity, no snap.
        if (playerMovingForward)
        {
            ...
            while (yawDiff < -MathHelper.Pi) yawDiff += MathHelper.TwoPi;
            _smoothedYaw = SmoothDampAngle(_smoothedYaw, _smoothedYaw + yawDiff, ref _yawVelocity, 0.5f, dt);
        }
        else
        {
            _yawVelocity = 0f;
        ...
        // Proactive obstacle detection: scan area around player for nearby trees/tall objects
        float targetObstacleBoost = 0f;
        if (TerrainHeightSampler != null)
        {
            float maxNearbyHeight = 0f;
            // Sample in a ring around the player position
            for (int i = 0; i < ObstacleProbePoints; i++)
            {
                float angle = i * MathHelper.TwoPi / ObstacleProbePoints;
                for (float r = 2f; r <= ObstacleProbeRadius; r += 3f)
                {
                    var probe = new Vector3(
                        targetPos.X + MathF.Cos(angle) * r,
                        targetPos.Y,
                        targetPos.Z + MathF.Sin(angle) * r);
                    float h = TerrainHeightSampler(probe);
                    if (h > maxNearbyHeight)
                        maxNearbyHeight = h;
                }
            }
            // Also check at the player position itself
            float playerH = TerrainHeightSampler(targetPos);
            if (playerH > maxNearbyHeight) maxNearbyHeight = playerH;

            if (maxNearbyHeight > 1f)
                targetObstacleBoost = maxNearbyHeight;
        }
        bool boostDescending = targetObstacleBoost < _smoothedObstacleBoost;
        float boostSmooth = boostDescending ? ObstacleBoostFallSmoothTime : ObstacleBoostRiseSmoothTime;
        _smoothedObstacleBoost = SmoothDamp(_smoothedObstacleBoost, targetObstacleBoost, ref _obstacleBoostVelocity, boostSmooth, dt);

        // Apply obstacle boost: raise camera distance and steepen pitch when near tall objects
        float obstacleDistBoost = _smoothedObstacleBoost > 1f ? _smoothedObstacleBoost * 0.6f : 0f;
        float obstaclePitchBoost = _smoothedObstacleBoost > 1f ? -MathHelper.Clamp(_smoothedObstacleBoost * 0.08f, 0f, 0.5f) : 0f;

        float desiredDist = _dist + runDistOffset + deployOffset + elevDistOffset + obstacleDistBoost;
        ...
        float effectivePitch = _pitch + elevPitchOffset + obstaclePitchBoost;
        ...
        // Hard safety net: raycast from lookAt to camera — push above any obstacle along the path
        if (TerrainHeightSampler != null)
        {
            const int RaySamples = 10;
            const float ClearanceAboveObstacle = 2.0f;
            float maxObstacleY = float.MinValue;

            for (int i = 0; i <= RaySamples; i++)
            {
                float t = i / (float)RaySamples;
                var samplePos = Vector3.Lerp(lookAt, camPos, t);
                float h = TerrainHeightSampler(samplePos);
                if (h > maxObstacleY)
                    maxObstacleY = h;
            }

            float minCamY = maxObstacleY + ClearanceAboveObstacle;
            if (camPos.Y < minCamY)
                camPos.Y = minCamY;
        }
